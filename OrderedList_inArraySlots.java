...
public class OrderedList_inArraySlots implements OrderedList {
  ...
  public OrderedList_inArraySlots(ArrayList<Integer> unordered) {
    ...
    for (int i = 0; i < unordered.size(); i++) {
      int champ = champIndex(unordered);
      add(unordered.get(champ));
      unordered.set(champ, null);
    }
  }
   ...
   * 
   * @return the index of the smallest of the elements, ignoring null elements,
   *         and using the classic reigning champ algorithm
   */
  private int champIndex(ArrayList<Integer> challengers) {
    int smallestIndex = -1;
    int i = 0;
    for (; i < challengers.size(); i++) {
      if (challengers.get(i) != null && (smallestIndex == -1 || challengers.get(smallestIndex).compareTo(challengers.get(i)) > 1)) {
        smallestIndex = i;
      }
    }
  ...
  /**
   * @return the index of any occurrence of \findMe in this list, or -1 if \findMe
  ...
  /**
   * @return the indexOf value, calculated recursively [Holmes's comments
   *         temporarily elided, so as to avoid spoilers for hw60_16]
   */
  private int indexOf_recursive(Integer findMe, int low, int hi // inclusive
  ) {
  ...
  public boolean add(Integer value) {
    int dest = 0;
    for (; dest < list_iAS.size() && list_iAS.get(dest) < value; dest++)
      ;
   ...
   *         you should NOT complicate your code to check whether the condition
   *         was violated.)
   */
